...
generate_schedule(Schedule) :-
    assign_all_courses([], Schedule).

assign_all_courses(Schedule, Schedule) :-
    all_courses_assigned(Schedule).
assign_all_courses(Partial, Schedule) :-
    course_sessions(Course, _),
    \+ assigned_course(Course, Partial),
    assign_sessions(Course, 1, Partial, Updated),
    assign_all_courses(Updated, Schedule).
...
assigned_course(Course, [assign(Course, _, _, _) | _]).
assigned_course(Course, [assign(OtherCourse, _, _, _) | Rest]) :-
    Course \= OtherCourse,
    assigned_course(Course, Rest).

all_courses_assigned(Schedule) :-
    \+ missing_course(Schedule).
