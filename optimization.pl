:- module(optimization, [
    ...
    best_schedule_exhaustive/2,
    compare_optimization_methods/2
]).
...
% Exhaustive optimization stores all scored schedules in memory.
best_schedule_exhaustive(BestSchedule, BestScore) :-
    setof(Score-Schedule,
          scored_schedule(Schedule, Score),
          [BestScore-BestSchedule | _]).

% Branch and Bound keeps only the current best complete schedule.
% This is memory-friendlier than storing every scored schedule.
update_best(Schedule, Score) :-
    best_solution(_, CurrentBestScore),
    Score < CurrentBestScore,
    !,
    retractall(best_solution(_, _)),
    assertz(best_solution(Schedule, Score)).
update_best(_, _).

% This version still evaluates complete schedules generated by generate_schedule/1.
% Stronger pruning would require integrating score bounds into recursive construction.
best_schedule_with_score(_BestSchedule, _BestScore) :-
    retractall(best_solution(_, _)),
    assertz(best_solution(none, 1000000000)),
    generate_schedule(Schedule),
    score(Schedule, Score),
    update_best(Schedule, Score),
    fail.
...
best_schedule(BestSchedule) :-
    best_schedule_with_score(BestSchedule, _).

% Both methods should return the same optimal score on the same dataset.
compare_optimization_methods(ExhaustiveScore, BranchBoundScore) :-
    best_schedule_exhaustive(_, ExhaustiveScore),
    best_schedule_with_score(_, BranchBoundScore).
